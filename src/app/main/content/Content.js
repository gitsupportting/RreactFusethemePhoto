...
class Content extends Component {
  ...
  onChangeFileArticle (e) {
    for (let i = 0; i < e.target.files.length; i++) {
      ...
      const url =
        'http://165.227.81.153:3005/files/otherarticleimg/' + imageFileName
      const options = {
        headers: {
          'x-pos-user-token': token,
          'Content-Type': 'application/x-www-form-urlencoded'
  ...
  handleOpen () {
    ...
    const url =
      'http://165.227.81.153:3005/getdata/addarticle/' + this.state.mainImage
    const options = {
      headers: {
        'x-pos-user-token': token,
        'Content-Type': 'application/x-www-form-urlencoded'
  ...
  aboutClickOpen (e, imagesource) {
    this.setState({
      imagesource: imagesource,
      openModalAboutArticle: true
    })
  }
  aboutClose () {
    this.setState({
  ...
  aboutOpen () {
    const { imagesource } = this.state
    var article_id = imagesource.article_id
    const url = 'http://165.227.81.153:3005/getdata/aboutus'
    token = localStorage.getItem('token') || ''
    const headers = {
      'Content-Type': 'application/json',
      'x-pos-user-token': token
  ...
  editOpen () {
    ...
    const url =
      'http://165.227.81.153:3005/getdata/editarticle/' + this.state.mainImage
    const options = {
      headers: {
        'x-pos-user-token': token,
        'Content-Type': 'application/x-www-form-urlencoded'
          ...
          <tr key={index}>
            ...
            <td>
              <Button
                ...
                onClick={e => this.openFileDgArticle(e, imagesource.article_id)}
              >
              ...
              <input
                ...
                style={{ display: 'none' }}
                onChange={e =>
                  this.onChangeFileArticle(e, imagesource.article_id)
                }
              />
            </td>
            <td>
              <Button
                class='toggle-button'
                id='centered-toggle-button'
                onClick={e => this.viewImagesClickOpen(e, imagesource)}
              >
                {translations[this.state.selectedLan]['_VIEW']}
              </Button>
            </td>
            <td>
              <Button
                class='toggle-button'
                id='centered-toggle-button'
                onClick={e => this.editClickOpen(e, imagesource)}
              >
                {translations[this.state.selectedLan]['_EDIT']}
              </Button>
            </td>
            <td>
              <Button
                class='toggle-button'
                id='centered-toggle-button'
                onClick={e => this.deleteClickOpen(e, imagesource)}
              >
                {translations[this.state.selectedLan]['_DELETE']}
              </Button>
            </td>
            <td>
              <Button
                class='toggle-button'
                id='centered-toggle-button'
                onClick={e => this.aboutClickOpen(e, imagesource)}
              >
                {translations[this.state.selectedLan]['_ABOUT']}
              </Button>
            </td>
          </tr>
        )
  ...
  handleChangeRowsPerPage = event => {
    ...
    const currentPage =
      rowsPerPage >= totalElements ? 0 : this.state.currentPage
    ...
    return (
      <FusePageSimple
        ...
        content={
          ...
          >
            {!this.state.openModal && !this.state.editArticle && (
              <h1 style={{ textAlign: 'center' }}>
                {translations[this.state.selectedLan]['_ARTICLES']}
              </h1>
            )}
            <br />
            <br />
            ...
            {this.state.openModal && (
              <div>
                <h1 style={{ position: 'absolute', top: 30, right: '38vw' }}>
                  {translations[this.state.selectedLan]['_CONTENT_MODAL_TITLE']}
                </h1>
                {this.state.selectedLan === '0' && (
                  <div style={{ marginBottom: 20, textAlign: 'left' }}>
                    ...
                    >
                      <InputLabel htmlFor='grouped-native-select'>
                        {translations[this.state.selectedLan]['_STATUS']}
                      </InputLabel>
                      <Select
                        native
                        defaultValue=''
                      ...
                      >
                        <option value={0}>
                          {translations[this.state.selectedLan]['_FALSE']}
                        </option>
                        <option value={1}>
                          {translations[this.state.selectedLan]['_TRUE']}
                        </option>
                      </Select>
                    </FormControl>
                    <FormControl
                      ...
                      style={{ marginLeft: '3vw', marginRight: '3vw' }}
                    >
                      <InputLabel htmlFor='grouped-native-select_sticky'>
                        {translations[this.state.selectedLan]['_STICKY']}
                      </InputLabel>
                      <Select
                        native
                        defaultValue=''
                        input={
                          <Input
                            id='grouped-native-select_sticky'
                            name='sticky'
            ...
            {this.state.editArticle && (
              <div>
                <h1 style={{ position: 'absolute', top: 30, right: '38vw' }}>
                  {translations[this.state.selectedLan]['_EDIT_ARTICLE_INFO']}
                </h1>
                {this.state.selectedLan === '0' && (
                  <div style={{ marginBottom: 20, textAlign: 'left' }}>
                    <TextField
                ...
                >
                  ...
                  {this.state.tempFile !== null && (
                    <img src={this.state.tempFile} width='50' height='50' />
                  )}
                  {this.state.tempFile === null && (
                    <img
                      src={
            ...
            {!this.state.openModal && !this.state.editArticle && (
              <div>
                <table id='usersDatas'>
                  {this.state.usersDatas !== [] && this.state.isUploaded && (
                    <tbody>
                      <tr>
                        <th>{translations[this.state.selectedLan]['_NO']}</th>
                        <th>
                          {translations[this.state.selectedLan]['_TITLE']}
                        </th>
                        <th>
                          {translations[this.state.selectedLan]['_STATUS']}
                        </th>
                        ...
                        <th>
                          {translations[this.state.selectedLan]['_DATE_TIME']}
                        </th>
                        <th>
                          {translations[this.state.selectedLan]['_MAIN_IMAGE']}
                        </th>
                        ...
                        <th>
                          {translations[this.state.selectedLan]['_VIEW_IMAGES']}
                        </th>
                        <th>{translations[this.state.selectedLan]['_EDIT']}</th>
                        <th>
                          {
                            translations[this.state.selectedLan][
                              '_DELETE_ARTICLE'
                            ]
                          }
                        </th>
            ...
            >
              <DialogTitle id='alert-dialog-title'>
                {translations[this.state.selectedLan]['_ARTICLE_DELETE']}
              </DialogTitle>
              <DialogActions>
            ...
            <Dialog
              open={this.state.openModalAboutArticle}
            ...
            >
              ...
              <DialogActions>
                <Button onClick={this.aboutClose} color='primary'>
                  {translations[this.state.selectedLan]['_DISAGREE']}
                </Button>
                <Button onClick={this.aboutOpen} color='primary' autoFocus>
                  {translations[this.state.selectedLan]['_AGREE']}
                </Button>
              </DialogActions>
            </Dialog>
